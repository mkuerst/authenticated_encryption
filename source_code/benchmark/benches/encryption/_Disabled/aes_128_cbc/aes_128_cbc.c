...
//size_t encrypt(const uint8_t *in, uint8_t *out, const size_t bytes)
...
{
    (void) mac; // Unused
    int hacked_value_to_adjust_ciphertect_len = 16;
    ...
    EVP_DecryptUpdate(ctx, out, &len, in, bytes + hacked_value_to_adjust_ciphertect_len);
    plaintext_len = len;
