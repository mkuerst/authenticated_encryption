...
inline void PartialXor(const __m256i val, const uint8_t *Src, uint8_t *Dest,
                       uint64_t Size) {
...
inline void PartialStore(const __m256i val, uint8_t *Dest, uint64_t Size) {
  _Alignas(32) uint8_t BuffForPartialOp[32];
  _mm256_storeu_si256((__m256i *)(BuffForPartialOp), val);
  memcpy(Dest, BuffForPartialOp, Size);
}

inline __m256i RotateLeft7(const __m256i val) {
  return _mm256_or_si256(_mm256_slli_epi32(val, 7),
                         _mm256_srli_epi32(val, 32 - 7));
}

inline __m256i RotateLeft12(const __m256i val) {
  return _mm256_or_si256(_mm256_slli_epi32(val, 12),
                         _mm256_srli_epi32(val, 32 - 12));
}

inline __m256i RotateLeft8(const __m256i val) {
  const __m256i mask =
...
inline __m256i RotateLeft16(const __m256i val) {
  const __m256i mask =
...
void ChaCha20EncryptBytes(uint8_t *__restrict__ state,
  ...
  const __m256i state0 = _mm256_broadcastsi128_si256(_mm_set_epi32(
      1797285236, 2036477234, 857760878, 1634760805)); //"expand 32-byte k"
  const __m256i state1 =
      ...
      if (In) // encrypt
      {
        ...
        _mm256_storeu_si256(
            ...
            _mm256_xor_si256(tmp,
                             _mm256_loadu_si256((const __m256i *)(CurrentIn))));
        RemainingBytes -= 32;
        if (RemainingBytes == 0) {
          ChaCha20AddCounter(state, 1);
          return;
