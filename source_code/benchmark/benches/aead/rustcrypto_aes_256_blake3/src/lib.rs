...
mod aes256blake3;
use aes256blake3::Aes256Blake3;

// // Type provided by RustCrypto::Aes256Gcm
// use aes_gcm::{aead::KeyInit, AeadInPlace, Nonce, Tag};
// type Aes256Gcm = aes_gcm::Aes256Gcm;

// // SAME AS RustCrypto::Aes256Gcm
// use aes_gcm::{aead::{self, KeyInit, consts::U12}, AeadInPlace, Nonce, Tag, aes::Aes256};
// type Aes256Gcm = aes_gcm::AesGcm<Aes256, U12, aead::consts::U16>;

// // SAME AS Cryptopp::Aes256Gcm (and OpenSSL)
// use aes_gcm::{aead::{KeyInit, consts::U16}, AeadInPlace, Nonce, Tag, aes::Aes256};
// type Aes256Gcm = aes_gcm::AesGcm<Aes256, U16, U16>;

// use aes_gcm::{aead::{KeyInit, consts::U16}, AeadInPlace, Nonce, Tag, aes::Aes256};
// type Aes256Gcm = aes_gcm::AesGcm<Aes256, U16, U16>; // SAME AS Cryptopp::Aes256Gcm (and OpenSSL)

use aes_gcm::{aead::KeyInit, AeadInPlace, Nonce, Tag};
type Aes256Gcm = Aes256Blake3;
...
unsafe fn aead_encrypt_internal(
    ...
    let mac: &mut [u8] = &mut *slice_from_raw_parts_mut(mac, MAC_LEN);
    let key: &[u8] = &*slice_from_raw_parts(key, KEY_LEN);
    let nonce: &[u8] = &*slice_from_raw_parts(nonce, NONCE_LEN);
...
fn encrypt_in_place(
    ...
    assert!(output.len() == plaintext.len());
...
unsafe fn aead_decrypt_internal(
    ...
    let input: &[u8] = &*slice_from_raw_parts(in_buf, bytes);
    let mac: &[u8] = &*slice_from_raw_parts(mac, MAC_LEN);
    let output: &mut [u8] = &mut *slice_from_raw_parts_mut(out_buf, bytes);
    let key: &[u8] = &*slice_from_raw_parts(key, KEY_LEN);
    let nonce: &[u8] = &*slice_from_raw_parts(nonce, NONCE_LEN);
...
fn decrypt_in_place(
    ...
    assert!(output.len() == ciphertext.len());
    output.clone_from_slice(ciphertext);
    cipher
