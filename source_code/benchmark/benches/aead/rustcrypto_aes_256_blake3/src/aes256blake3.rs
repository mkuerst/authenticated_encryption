...
pub use aes_gcm::aead::{self, AeadCore, AeadInPlace, Error, Key, KeyInit, KeySizeUser};
...
pub type Aes256Blake3 = AesGcm<Aes256, U16>;
    ...
    TagSize: self::TagSize,
{
    fn from(cipher: Aes) -> Self {
        // eprintln!("ENTERED AesGcm::from(cipher: Aes)");
        // let mut ghash_key = ghash::Key::default();
        // cipher.encrypt_block(&mut ghash_key);

        let ghash = blake3::Hasher::new();
        // eprintln!("EXIT AesGcm::from(cipher: Aes)");
    ...
    TagSize: self::TagSize,
{
    fn encrypt_in_place_detached(
        ...
        // eprintln!("ENTERED encrypt_in_place_detached");
    ...
    fn decrypt_in_place_detached(
        ...
        // eprintln!("ENTERED decrypt_in_place_detached");
    ...
    TagSize: self::TagSize,
{
    ...
    fn init_ctr(&self, nonce: &Nonce<NonceSize>) -> (Ctr32BE<&Aes>, Block) {
        // eprintln!("ENTERED AesGcm::init_ctr");
        ...
        //     ghash.finalize()
        // };
        // let j0: cipher::Iv<_> = self.ghash.clone().update(b"0000000000000000").finalize().into();
        // let j0: cipher::Iv<Ctr32BE<Aes>> = cipher::Iv::from_slice(b"0000000000000000");
        let j0: GenericArray<_, _> = Default::default(); // TODO
        // assert!(NonceSize::to_usize() == 16);
        // assert!(j0.as_slice().len() == 16);

        let mut ctr = Ctr32BE::inner_iv_init(&self.cipher, &j0);
        let mut tag_mask = Block::default();
        ctr.write_keystream_block(&mut tag_mask);
        // eprintln!("EXIT AesGcm::init_ctr");
        (ctr, tag_mask)
    }
    ...
    fn compute_tag(&self, mask: Block, associated_data: &[u8], buffer: &[u8]) -> Tag {
        // eprintln!("ENTERED AesGcm::compute_tag");
        let mut ghash = self.ghash.clone();
        ghash.update(associated_data);
        ghash.update(buffer);
        ...
        // for (a, b) in tag.as_bytes().iter_mut().zip(mask.as_slice()) {
        //     *a ^= *b;
        // }
        
        let tag = ghash.finalize();
        // assert!(tag.as_bytes().len() == 32);
        // assert!(mask.as_slice().len() == 16);
        
        // TODO: second half of hash currently unused
        Tag::from_exact_iter(tag.as_bytes()[..16].iter().zip(mask.as_slice()).map(|(a, b)| a ^ b)).unwrap()

        // // Tag::from_slice(tag.as_bytes())
        // Tag::from_exact_iter(tag.as_bytes().iter().map(|x| *x)).unwrap()
    }
}
